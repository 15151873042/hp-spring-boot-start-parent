...
import hp.boot.mybatis.mapping.BoundSqlSource;
...
public class MSUtil {
    ...
    public static MappedStatement newCountMappedStatement(MappedStatement ms) {
        Builder builder = new Builder(ms.getConfiguration(), ms.getId() + "_COUNT", ms.getSqlSource(), ms.getSqlCommandType());
    ...
    public static MappedStatement copyFromMappedStatement(MappedStatement ms,BoundSql boundSql) {  
      Builder builder = new Builder(ms.getConfiguration(),ms.getId(),new BoundSqlSource(boundSql),ms.getSqlCommandType());  
      builder.resource(ms.getResource());  
      builder.fetchSize(ms.getFetchSize());  
      builder.statementType(ms.getStatementType());  
      builder.keyGenerator(ms.getKeyGenerator());  
